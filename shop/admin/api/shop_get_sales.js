...
function shopGetSales() {
    this.Service = async function (version, param, callback) {
        ...
        try {
            ...
            // 3.
            // sql = "select count(id),postage,create_time,select_card_id,tradeId from `order` where state >= ? and state != ? and pay_state = ? and create_time >= ? and create_time <= ? group by tradeId order by create_time"
            // row = await db.Query(sql, [1,5,0, new Date(param['start_time']), new Date(param['end_time'])])
			sql = "select count(id),postage,create_time,select_card_id,tradeId from `order` where id in (select order_id from paid where state = ?) and pay_state = ? and create_time >= ? and create_time <= ? group by tradeId order by create_time"
            row = await db.Query(sql, [1,0, new Date(param['start_time']), new Date(param['end_time'])])
            if (row.length > 0) {
                data.order = row
                for (let i in data.order) {
                    // let reduce_cost = 0
                    // if (data.order[i].select_card_id) {
                    //     sql = "select cash from card_info where card_id = (select card_id from card where id = ?)"
                    //     row = await db.Query(sql, data.order[i].select_card_id)
                    ...
                    for (let j in row) {
                        ...
                        // total_price = (Number(total_price) + Number((row[j].single_price * row[j].number).toFixed(2))).toFixed(2)
                        // console.info(total_price)
                    }
                    data.order[i].total_price = (Number(total_price) + data.order[i].postage).toFixed(2)
                    // data.order[i].total_price = (Number(total_price) + data.order[i].postage - reduce_cost).toFixed(2)
                }
            } else {
                data.order = []
            }
