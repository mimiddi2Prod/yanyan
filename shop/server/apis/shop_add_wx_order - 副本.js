...
async function SHOPAddWXOrder(getParam) {
    ...
    for (let i in getParam.order) {
        ...
        if (param["disCountPrice"]) {
            sql = "insert into `order`(user_id,open_id,item_id,goodsname,image,`number`,state,address_text,tel,receiver,single_price,total_price,postage,tradeId,have_cost_integral,integral_price,create_time,update_time,select_card_id,discount_price,discount_price_total)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,?,?,?)"
            ...
            sql = "insert into `order`(user_id,open_id,item_id,goodsname,image,`number`,state,address_text,tel,receiver,single_price,total_price,postage,tradeId,have_cost_integral,integral_price,create_time,update_time,select_card_id)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,?)"
            row = await tool.query(sql, [param["user_id"], param["open_id"], param["item_id"], param["name"], param["image"], param["number"], param["state"], param["address_text"], param["tel"], param["receiver"], param["single_price"], sumPrice, param["postage"], getParam["tradeId"], param["have_cost_integral"], param["integral_price"], param["select_card_id"]])
        }
...
function getAverageDisCountPrice(data) {
    ...
    //         order[i].disCountPrice = Number((Number(order[i].single_price) - Number((Number(emaining) / order[i].number).toFixed(2))).toFixed(2))
    //         break
    //     }
    ...
    //     cost = Number((Number((Math.round(Number((Number(order[i].reduce_cost) / Number(total_price)).toFixed(2)) * 100) / 100).toFixed(2)) * Number(order[i].single_price)).toFixed(2))
    //
    //     emaining = Number((Number(emaining) - Number((cost * Number(order[i].number)).toFixed(2))).toFixed(2))
    //     order[i].disCountPrice = Number((Number(order[i].single_price) - cost).toFixed(2))
    //
