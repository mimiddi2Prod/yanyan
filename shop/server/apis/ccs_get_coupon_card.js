...
function CCSGetCouponCard() {
    ...
    this.Run = async function (ver, param, res) {
        ...
        if (!param["openid"]) {
            ...
            var sql = ""
            try {
                // type : opening / consumption(是否是新用户)
                if (param["isNewCustomer"]) {
                    // 消费后 除了通常的消费红包，还有新用户红包
                    sql = "select * from card_info where `type` = ? || `type` = ?"
                    row = await tool.query(sql, [param["type"], "newCustomer"])
                } else if (param["type"] == "opening" || param["type"] == "consumption") {
                    sql = "select * from card_info where `type` = ?"
                    row = await tool.query(sql, param["type"])
                } else if (param["type"] == "share") {
                    sql = "select * from share_record where share_id = (select id from user where open_id = ?)"
                    row = await tool.query(sql, param["openid"])
                    let shareNumber = row.length
                    // shareNumber : 用户已分享的新用户人数
                    sql = "select * from card_info where `type` = ? limit ?,?"
                    row = await tool.query(sql, [param["type"], 0, shareNumber])
                }
                if (row.length > 0) {
